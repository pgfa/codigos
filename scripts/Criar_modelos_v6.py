...
Info: Codigo criado para testar diversos modelos de Rede Neural do tipo LSTM e salvar o modelo com melhor resultado
...
epocas = [n for n in range(100, 700, 100)]

#Opções de batchs
batchSizes = [32, 64]

#Quantidade de numeros (timestamps) anteriores para prever um valor
anteriores = [400, 500] 
...
#Tipode de LSTM que serão testadas: Vanilla e Bidirectional
tipos = [1, 2]

#Nome de arquivos associados a este arquivo
file1 = 'novo-outono4.h5'
file2 = 'novo-outono4.txt'
...
base = pd.read_csv('outono-treinamento.csv')
...
#Separar qual vai ser o valor para fazer previsão, será da coluna 1 até a última, serão os previsores
base_treinamento = base.iloc[:,0:colunas].values #Todas colunas
...
# Utilizar determinado número de timestamps anteriores para prever valor da ação
for anterior in anteriores:
    previsores = [] #lista
    preco_real = [] #lista
    
    # Deve começar a tentar prever a partir da posição X (variavel "anteriores"), pois os primeiros valores são para tentar prever. 
    for i in range(anterior, linhas): # append é para adicionar dado na lista
        previsores.append(base_treinamento_normalizada[i-anterior:i, 0:colunas])# i = 0,1,2,3... . vai até  valor de "colunas"
        # preenche preço real
        preco_real.append(base_treinamento_normalizada[i, 0]) # i = 90 , preço das 90 datas anteriores, 0 = temeratura do ar
    ...
    #Testar em outra base
    base_teste = pd.read_csv('outono-teste.csv')
    #Extrair somente variável Temp, pegando apenas primeira coluna
    preco_real_teste = base_teste.iloc[:,0:1].values # Pegando coluna 1, Temp
    
    #Definir quantidade de linhas do arquivo de teste
    ...
    #Vamos precisar dos anteriores, para isso podemos concatenar
    base_completa = pd.concat(frames) 
    
    #considerar de deve Apagar coluna "Dates", muitas vezes não é usado
    #base_completa = base_completa.drop('Date', axis = 1)
    
    entradas = base_completa[len(base_completa) - len(base_teste) - anterior:].values #.values converte para formato numpy array, -400 (é um exemplo) para indicar onde começa a checar
    ...
    X_teste = [] # lista fazia
    for i in range(anterior, total): # anterior + registros
        X_teste.append(entradas[i-anterior:i, 0:colunas])#coluna 0 até última
    X_teste = np.array(X_teste)
    #Agora temos X colunas e Y registros
    #############################################################################################
    ...
    for c in range(len(configuracoes)):
        ...
        for epoca in epocas:
            for batch in batchSizes:
                ...
                plt.savefig('Outono.'+str(len(configuracao))+'Cam.'+str(configuracao[0].neuronio)+'Neu.'+nome_tipo+'Tipo.Temp.'+nome+'.Ant.'+str(anterior)+'.png')
                #plt.show()
                plt.close()
