...
@author: cesar, valfride, lucas, raphael
"""
####
...
from keras.layers import Dense, Dropout, LSTM, Flatten # LSTM adicionado na aula 70
...
file1 = R'inverno20112016_90_10.h5'
file2 = R'inverno20112016_90_10.txt'
...
#Valor base para teste de Wilcoxon
alpha = 0.05

#Bases
base = pd.read_csv(R'inverno20112016-treinamento.csv')
file_teste = R'inverno20112016-teste.csv'
    ...
    for c in range(len(configuracoes)):
        ...
        for epoca in epocas:
            for batch in batchSizes:
                ...
                lstm = LSTM(units=configuracao[0].neuronio, return_sequences=configuracao[0].return_sequences, input_shape=(None, 300, 200))
                ...
                regressor.add(Dense(units = 1, input_shape=(5,), activation = 'sigmoid'))
                regressor.build((None, 300, 28))
                regressor.load_weights(R'inverno20112016_90_10.h5')
                ...
                #Bloco Wilcoxon#####################################
                #converter variaveis para 1D
                previsoes2 = np.array(previsoes)
                previsoes2 = previsoes.ravel()
                #converter variaveis para 1D
                preco_real_teste2 = np.array(preco_real_teste)
                preco_real_teste2 = preco_real_teste.ravel()
                # Agora sim o teste de Wilcoxon
                stat, p = wilcoxon(preco_real_teste2, previsoes2)
                print('Wilcoxon Statistics=%.3f, p=%.3f' % (stat, p))
                #Formatar limitando em 3 casa decimais após vírgula
                #print('O valor de stat formatado é {:.3f}'.format(stat))
                #print('O valor de p formatado é {:.3f}'.format(p))
                #Checar se p é maior que 0,05
                if p > alpha:
                ...
                #Kendall-----------------------------------------
                coef, p2 = kendalltau(preco_real_teste2, previsoes2)
                ...
                #Spearman++++++++++++++++++++++++++++++++++++++++++
                coef2, rank_corr = spearmanr(preco_real_teste, previsoes)
                #
                print('Spearman: ', rank_corr)
                #
                if rank_corr > alpha:
                    print('Samples are uncorrelated (fail to reject H0) p=%.3f' % rank_corr)
                else:
                    print('Samples are correlated (reject H0) p=%.3f' % rank_corr)
                ...
                f.write('--------------------------' '\n')
                f.write('Wilcoxon Stats = ' + repr(stat) + '\n')
                f.write('Wilcoxon p = ' + repr(p) + '\n')
                f.write('--------------------------' '\n')
                f.write('Kendall coefic = ' + repr(coef) + '\n')
                f.write('Kendall pvalue = ' + repr(p2) + '\n')
                f.write('--------------------------' '\n')
                f.write('Spearman coef = ' + repr(coef2) + '\n')
                f.write('Spearman p = ' + repr(rank_corr) + '\n')                
                f.write('--------------------------' '\n')
                ...
                plt.savefig('v6.wilcoxon.'+str(len(configuracao))+'Cam.'+str(configuracao[0].neuronio)+'Neu.'+nome_tipo+'Tipo.Temp.'+nome+'.Ant.'+str(anterior)+'.png')
                #plt.show()
                plt.close()
